  ...
  async broadcastToNearbyShippers(deliveryId: string, pickupLng: number, pickupLat: number) {
    ...
    const shippers = await this.prisma.$queryRaw<NearbyShipper[]>(Prisma.sql`
      ...
      WHERE vp.is_available = TRUE
        ...
        -- Loại người đã TỪ CHỐI đơn này (không mời lại) để nhường slot cho người
        -- xa hơn — nếu không, 5 người gần nhất cùng từ chối sẽ kẹt đơn vĩnh viễn
        -- (upsert offer 'rejected' là no-op, không bao giờ có offer mới).
        AND NOT EXISTS (
          SELECT 1 FROM shipper_task_offers o
          WHERE o.delivery_id = ${deliveryId}::uuid
            AND o.shipper_id = vp.id
            AND o.status IN ('rejected', 'accepted')
        )
    ...
    // Upsert TẤT CẢ offer trong MỘT câu SQL (1 round-trip) — không dùng interactive
    // transaction: với DB cloud (Supabase), 5 câu tuần tự vượt timeout 5s → P2028.
    await this.prisma.$executeRaw(Prisma.sql`
      INSERT INTO shipper_task_offers (delivery_id, shipper_id, status, expires_at)
      VALUES ${Prisma.join(
        shippers.map(
          (s) =>
            Prisma.sql`(${deliveryId}::uuid, ${s.id}::uuid, 'pending'::offer_status, ${expiresAt.toISOString()}::timestamptz)`,
        ),
      )}
  ...
  private async failUnassignedTimeouts(): Promise<number> {
    ...
    for (const d of stale) {
      await this.prisma.$transaction([
        this.prisma.delivery.update({
          where: { id: d.id },
          data: { status: 'failed', failedReason: 'Không có tình nguyện viên nào nhận đơn trong thời gian tìm kiếm.' },
        }),
        this.prisma.shipperTaskOffer.updateMany({
        ...
        // Hàng chưa rời bếp → hoàn số lượng cho listing
        this.prisma.$executeRaw(Prisma.sql`
          UPDATE food_listings
          SET
            quantity_remaining = LEAST(quantity_total, quantity_remaining + ${Number(d.reservation.quantity)}),
      ...
      // Báo người nhận (chuông + socket) để họ biết mà đặt lại
