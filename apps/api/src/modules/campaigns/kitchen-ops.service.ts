...
const SAFETY_RESULT_VN: Record<string, string> = {
  pass: 'Đạt',
  warning: 'Cảnh báo',
  fail: 'Không đạt',
};
const ACTIVE_WORK_ASSIGNMENT_STATUSES = [
  'assigned',
  'checked_in',
  'in_progress',
] as const;
...
export class KitchenOpsService {
  ...
  private async assertAssignedAs(
    campaignId: string,
    userId: string,
    role: 'chef' | 'waiter',
  ) {
    ...
    if (volunteer.user.status !== 'active') {
      throw new ForbiddenException(
        'Tài khoản của bạn chưa ở trạng thái hoạt động, không thể thao tác.',
      );
    }
    if (volunteer.specializations.length === 0) {
      throw new ForbiddenException(
        `Chuyên môn "${ROLE_VN[role]}" của bạn chưa được xác minh.`,
      );
    }
  ...
  async resolveShiftApplication(
    ...
    if (!role) {
      throw new BadRequestException(
        'Ca này là ca chung, vui lòng chọn vai trò bạn muốn đăng ký.',
      );
    }
  ...
  async addMenuItem(campaignId: string, userId: string, dto: AddMenuItemDto) {
    ...
    if (!dto.recipeId && !dto.customName) {
      throw new BadRequestException(
        'Cần chọn một công thức hoặc nhập tên món tự do.',
      );
    }
    if (dto.recipeId) {
    ...
    const [item] = await this.prisma.$transaction([
      this.prisma.campaignMenuItem.create({
        ...
        include: {
          recipe: { select: { name: true, servings: true, difficulty: true } },
        },
      }),
      ...(dto.recipeId
        ? [
  ...
  async removeMenuItem(itemId: string, userId: string) {
    const item = await this.prisma.campaignMenuItem.findUnique({
      where: { id: itemId },
    });
    if (!item)
  ...
  async createSafetyLog(
    ...
    if (!['open', 'in_progress'].includes(campaign.status)) {
      throw new BadRequestException(
        'Chỉ ghi nhật ký ATTP khi chiến dịch đang chuẩn bị/diễn ra.',
      );
    }
  ...
  async listSafetyLogs(campaignId: string) {
    return this.prisma.kitchenSafetyLog.findMany({
      ...
      include: {
        checkedBy: { select: { user: { select: { fullName: true } } } },
      },
    });
  }
  ...
  async createDistribution(
    ...
    if (!['open', 'in_progress', 'completed'].includes(campaign.status)) {
      throw new BadRequestException(
        'Chiến dịch không ở trạng thái cho phép ghi phân phát.',
      );
    }
    ...
    if (dto.peopleServed > dto.servingsServed) {
      throw new BadRequestException(
        'Số người nhận không thể lớn hơn số suất đã phát.',
      );
    }
    ...
    if ((dto.lng === undefined) !== (dto.lat === undefined)) {
      throw new BadRequestException(
        'Cần cả kinh độ (lng) và vĩ độ (lat) khi gắn vị trí phát.',
      );
    }
  ...
  async addFeedback(
    ...
    if (actor.role !== UserRole.VOLUNTEER || !actor.volunteerProfile) {
      throw new ForbiddenException(
        'Chỉ tình nguyện viên được phân công mới có thể ghi nhận phản hồi tại điểm phát.',
      );
    }
    ...
    if (dist.servedByVolunteerId !== actor.volunteerProfile.id) {
      throw new ForbiddenException(
        'Chỉ waiter đã ghi nhận đợt phát này mới có thể nhập phản hồi.',
      );
    }
  ...
  private async assertIndividualReceiver(userId: string) {
    ...
    if (
      receiver.user.status !== 'active' ||
      receiver.user.role !== UserRole.RECEIVER
    ) {
      throw new ForbiddenException(
        'Tài khoản của bạn chưa ở trạng thái hoạt động.',
      );
    ...
    if (receiver.isCharityOrg) {
      throw new ForbiddenException(
        'Tài khoản tổ chức không thể nhận suất ăn với vai trò người thụ hưởng.',
      );
    }
    return receiver;
  }
  ...
  async issueHandoffQr(userId: string) {
    ...
    const validMinutes = await this.systemConfig.getNumber(
      'HANDOFF_QR_VALIDITY_MINUTES',
    );
    ...
    const token = await this.prisma.$transaction(async (tx) => {
      ...
      const [row] = await tx.$queryRaw<
        { id: string; qr_token: string; qr_expires_at: Date }[]
      >(Prisma.sql`
    ...
    return {
      id: token.id,
      qrToken: token.qr_token,
      expiresAt: token.qr_expires_at,
    };
  }
  ...
  async scanHandoff(
    ...
    if (!dist || dist.campaignId !== campaignId) {
      throw new NotFoundException(
        'Không tìm thấy đợt phân phát trong chiến dịch này.',
      );
    }
    ...
    if (token.qrExpiresAt.getTime() <= Date.now()) {
      throw new BadRequestException(
        'Mã QR đã hết hạn, người nhận cần làm mới mã.',
      );
    }
    ...
    if (
      ...
      throw new ForbiddenException(
        'Tài khoản người nhận không ở trạng thái hoạt động.',
      );
    }
    if (receiver.isCharityOrg) {
      throw new ForbiddenException(
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (consumed.count !== 1) {
          ...
          throw new ConflictException(
            'Mã QR này vừa được sử dụng, vui lòng yêu cầu người nhận làm mới mã.',
          );
        }
      ...
      if (
        ...
        throw new ConflictException(
          'Người nhận này đã được ghi nhận tại đợt phát này.',
        );
      }
      throw e;
    }
      ...
      include: {
        ...
        distribution: {
          select: {
            ...
            distributedAt: true,
            campaign: {
              select: { id: true, title: true, kitchenAddress: true },
            },
          },
        },
      },
  ...
  async submitBeneficiaryFeedback(
    ...
    if (!handoff)
      throw new NotFoundException('Không tìm thấy biên nhận suất ăn.');
    if (handoff.receiverId !== receiver.id) {
      throw new ForbiddenException(
        'Bạn chỉ có thể gửi phản hồi cho suất ăn mình đã nhận.',
      );
    }
