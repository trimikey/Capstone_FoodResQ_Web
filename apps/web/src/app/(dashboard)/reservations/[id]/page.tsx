...
function ShipperProfilePhoto({
  ...
  const label = name || 'Tình nguyện viên';

  if (!src || failed) {
    return (
      <div
        className={`${sizeClass} rounded-full border border-neutral-100 bg-emerald-50 text-emerald-800 flex items-center justify-center font-extrabold shrink-0`}
      >
        {initials(label)}
      </div>
    );
  ...
  return (
    <img
      src={mediaUrl(src)}
      alt={label}
      className={`${sizeClass} rounded-full object-cover border border-neutral-100 shrink-0`}
...
function deliveryHeadline(
  ...
  if (status === 'failed' && failedReason) {
    return {
      ...
      desc: `${failedReason} Bạn có thể đặt lại đơn hoặc chọn tự đến lấy.`,
    };
  }
  switch (status) {
    case 'pending_assignment':
      return {
        title: 'Đang tìm tình nguyện viên',
        desc: 'Đang tìm người giao phù hợp gần bạn — vui lòng chờ trong giây lát.',
      };
    case 'assigned':
      return {
        title: 'Đã có tình nguyện viên nhận đơn',
        desc: 'Tình nguyện viên sắp đến điểm lấy hàng.',
      };
    case 'heading_to_provider':
      return {
        title: 'Đang đến lấy hàng',
        desc: 'Tình nguyện viên đang trên đường đến điểm lấy hàng.',
      };
    case 'qc_completed':
      return {
        title: 'Đã lấy hàng',
        desc: 'Tình nguyện viên đã lấy & kiểm tra hàng, chuẩn bị giao cho bạn.',
      };
    case 'in_transit':
      return {
        title: 'Đang giao hàng',
        desc: 'Tình nguyện viên đã lấy hàng và đang trên đường giao.',
      };
    case 'delivered':
      return {
        title: 'Đã giao thành công',
        desc: 'Cảm ơn bạn đã đồng hành cứu trợ thực phẩm cùng FoodResQ!',
      };
    case 'failed':
      return {
        title: 'Giao hàng thất bại',
        desc: 'Đơn giao gặp sự cố. Vui lòng liên hệ hỗ trợ để được hỗ trợ.',
      };
    case 'cancelled':
      return {
        title: 'Đã hủy tìm tình nguyện viên',
        desc: 'Bạn đã hủy tìm. Vui lòng đến địa chỉ nhận trong khung giờ đã chọn.',
      };
    default:
...
export default function ReservationDetailsPage() {
  ...
  useEffect(() => {
    if (fetchedData) {
      ...
      if (
        id.includes('pickup') ||
        id.includes('An-Nhien') ||
        id.includes('An_Nhien') ||
        id.includes('rq-') ||
        id.includes('RQ-')
      ) {
        setDeliveryMethod('pickup');
  ...
  useEffect(() => {
    const reservation = fetchedData as
      | { status?: string; listing?: { pickupEndTime?: string } }
      | null
      | undefined;
    if (
      reservation &&
      ...
      <div className="flex-1 flex flex-col items-center justify-center min-h-[70vh] bg-surface gap-md px-4 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-on-surface-variant">
            Vui lòng thử lại sau hoặc quay lại danh sách đơn.
          </p>
        </div>
        <Link
          href="/reservations"
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center min-h-[70vh] bg-surface gap-md px-4 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-on-surface-variant">
            Đơn nhận này không tồn tại hoặc bạn không có quyền xem.
          </p>
        </div>
        <Link
          href="/reservations"
  ...
  if (showTimeInfo && listing && deliveryMethod === 'pickup') {
    const isCompleted =
      ...
      fetchedData?.status === 'no_show';
    if (!isCompleted) {
      return (
        <ReservationTimeInfoPopup
          pickupStartTime={
            (fetchedData?.listing as { pickupStartTime?: string } | null)?.pickupStartTime ??
            mockListing?.pickupStartTime ??
            new Date().toISOString()
          }
          pickupEndTime={
            (fetchedData?.listing as { pickupEndTime?: string } | null)?.pickupEndTime ??
            mockListing?.pickupEndTime ??
            new Date().toISOString()
          }
  ...
  if (!reservation) {
    ...
    reservation = {
      ...
      quantity: 1,
      status:
        currentStep === 2 ? 'completed' : deliveryMethod === 'delivery' ? 'picked_up' : 'confirmed',
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      setChatHistory((prev) => [
        ...prev,
        { sender: 'shipper', text: 'Dạ vâng ạ, mình đang di chuyển rồi.' },
      ]);
    }, 1500);
  };
    ...
    <div className="min-h-screen bg-neutral-50 pb-20">
      ...
      <div className="bg-white border-b border-neutral-200 py-3 px-6">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center gap-xs text-xs font-medium text-neutral-500">
          ...
          <span className="material-symbols-outlined text-[14px]">chevron_right</span>
          <span className="min-w-0 max-w-full truncate text-neutral-800 font-semibold">
                      ...
                      <div className="mt-3 flex items-center gap-3">
                        {countdown !== null && countdown > 0 ? (
                          <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-full">
                            <span className="material-symbols-outlined text-amber-600 text-[16px] animate-pulse">
                              schedule
                            </span>
                ...
                <div className="mt-8 relative">
                  ...
                  <div className="absolute top-[9px] left-0 right-0 h-1 bg-neutral-100 rounded-full z-0">
                    <div
                      ...
                      style={{
                        width: isReservationClosed
                          ? '0%'
                          : `${((useRealDelivery ? realDeliveryStep! : currentStep === 2 ? 3 : 2) / 3) * 100}%`,
                      }}
                    />
                  </div>
                    ...
                    {[
                      ...
                      return (
                        <div key={idx} className="flex flex-col items-center">
                          ...
                          />
                          <span
                            className={`mt-2 font-bold text-xs ${isCompleted ? 'text-emerald-700' : 'text-neutral-400'}`}
                          >
                            {step.label}
                          </span>
                        </div>
            ...
            <div className="space-y-6">
              ...
              {isOrderClosed ? (
                <div className="bg-neutral-50 rounded-2xl border border-neutral-200 p-6 shadow-sm text-center flex flex-col items-center">
                  <span className="material-symbols-outlined text-neutral-400 text-[36px]">
                    cancel
                  </span>
                ...
                <div className="bg-emerald-50 rounded-2xl border border-emerald-200 p-6 shadow-sm text-center flex flex-col items-center">
                  ...
                  <div className="my-5 p-4 bg-white border border-emerald-100 rounded-2xl shadow-inner flex flex-col items-center gap-3">
                    <div className="border-4 border-neutral-800 rounded-xl overflow-hidden p-2.5 bg-neutral-900 flex flex-col items-center w-[160px] h-[190px]">
                      <div className="w-full text-[8px] text-white/50 text-center mb-1">
                        MÃ XÁC NHẬN
                      </div>
                      ...
                      <div className="w-full text-[8px] text-emerald-400 font-bold text-center mt-1.5">
                        {reservation.listing.orderId || '#RESQ-8821'}
                      </div>
                    </div>
                  </div>
              ...
              {isReservationClosed && !realShipper ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h5 className="font-bold text-neutral-800">
                        Không có tình nguyện viên nhận đơn
                      </h5>
                ...
                <div className="bg-white rounded-2xl border border-neutral-200 p-5 shadow-sm">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {(tracking as { deliveryScheduledAt?: string | null } | undefined)
                        ?.deliveryScheduledAt && (
                        <p className="mt-0.5 inline-flex items-center gap-1 rounded-full bg-sky-50 px-2 py-0.5 text-[11px] font-bold text-sky-700">
                          ...
                          Hẹn giao{' '}
                          {new Date(
                            (tracking as { deliveryScheduledAt?: string }).deliveryScheduledAt!,
                ...
                (() => {
                  ...
                  const shipperProfilePhotoUrl = useRealDelivery
                    ? realShipper?.profilePhotoUrl
                    : null;
                  return (
                    <div className="bg-white rounded-2xl border border-neutral-200 p-5 shadow-sm">
                      <h4 className="text-xs font-bold text-neutral-400 uppercase tracking-wider mb-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <ShipperProfilePhoto src={shipperProfilePhotoUrl} name={shipperName} />
                          <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() =>
                              useRealDelivery ? setChatShipperOpen(true) : setIsChatOpen(true)
                            }
              ...
              <div className="bg-white rounded-2xl border border-neutral-200 p-5 shadow-sm space-y-4">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-16 h-16 rounded-xl overflow-hidden bg-neutral-100 shrink-0">
                    <img
                      src={mediaUrl(
                        reservation.listing.imageUrls?.[0] || '/banh-mi-ngot-thap-cam.png',
                      )}
                      alt="Food"
                  ...
                  <div className="flex-1 min-w-0">
                    <h5 className="font-bold text-neutral-800 truncate">
                      {reservation.listing.title}
                    </h5>
                    <p className="text-xs text-neutral-500 mt-0.5">
                      Từ: {reservation.listing.provider.businessName}
                    </p>
                    ...
                    <span className="font-bold text-neutral-800">
                      {(() => {
                        ...
                        const pickupStart = new Date(
                          reservation.listing.pickupStartTime ?? reservation.createdAt,
                        ).getTime();
                        const pickupEnd = new Date(
                          reservation.listing.pickupEndTime ?? reservation.createdAt,
                        ).getTime();
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl border border-neutral-200 p-5 mb-6 shadow-sm flex items-center justify-center gap-8 max-w-2xl mx-auto">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-sm font-bold ${currentStep === 1 ? 'text-neutral-800' : 'text-neutral-400'}`}
                >
                  Đến điểm nhận
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${
                    currentStep === 2
                      ? 'bg-emerald-800 text-white animate-bounce'
                ...
                <span
                  className={`text-sm font-bold ${currentStep === 2 ? 'text-neutral-800' : 'text-neutral-400'}`}
                >
                  Xác nhận thành công
                </span>
              </div>
                  ...
                  <div className="p-5 border-b border-neutral-100 flex items-center justify-between">
                    <div>
                      <h3 className="font-bold text-lg text-neutral-800">
                        Xác nhận Nhận hàng Trực tiếp
                      </h3>
                      <p className="text-xs text-neutral-400 mt-0.5">
                        Vui lòng quét mã tại địa điểm nhà cung cấp
                      </p>
                    </div>
                  ...
                  <div className="bg-neutral-900 min-h-[420px] sm:min-h-0 sm:aspect-video relative flex items-center justify-center group overflow-hidden">
                    {isOrderClosed ? (
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-neutral-900 text-white p-6 text-center z-10">
                        <span className="material-symbols-outlined text-[48px] text-neutral-500">
                          block
                        </span>
                        ...
                        <p className="text-xs text-neutral-400 max-w-xs leading-relaxed">
                          Mã QR không còn hiệu lực. Suất ăn đã được trả lại cho nhà cung cấp để
                      ...
                      !isMock ? (
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-neutral-900 text-white p-6 text-center z-10">
                          ...
                          <p className="text-sm font-semibold max-w-xs">
                            Đưa mã QR này cho nhà cung cấp quét để xác nhận đã nhận hàng
                          </p>
                          {liveStatus === 'picked_up' ? (
                            <button
                              onClick={() => openProof('face')}
                        ...
                        <>
                          ...
                          <div
                            className="absolute inset-0 bg-cover bg-center filter blur-[1px] opacity-70"
                            style={{
                              backgroundImage:
                          ...
                          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/75 backdrop-blur px-5 py-2.5 rounded-full z-20 flex items-center gap-2 border border-white/10 text-white max-w-[90%]">
                            ...
                            <span className="text-xs font-medium">
                              Đưa mã QR của Nhà cung cấp vào khung
                            </span>
                          </div>

                          {/* Floating Click to Scan Simulation Trigger overlay */}
                          <div
                            className="absolute inset-0 bg-black/10 hover:bg-black/35 transition-all flex items-center justify-center cursor-pointer group"
                            onClick={handleSimulateScan}
                          >
                        ...
                        <div className="bg-white/10 border border-white/10 rounded-2xl p-4 flex items-center gap-3 text-white text-xs max-w-sm">
                          ...
                          <div className="text-left">
                            ...
                            <p className="text-[10px] text-neutral-300">
                              Điểm uy tín của bạn đã được cập nhật thành công
                            </p>
                          </div>
                        </div>
                        <Link
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-neutral-200 p-5 shadow-sm space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-14 h-14 rounded-xl overflow-hidden bg-neutral-100 shrink-0">
                      <img
                        src={
                          reservation.listing.imageUrls?.[0]
                            ? mediaUrl(reservation.listing.imageUrls[0])
                            : '/banh-mi-ngot-thap-cam.png'
                        }
                        alt="Food"
                        className="w-full h-full object-cover"
                ...
                <div className="bg-white rounded-2xl border border-neutral-200 p-5 shadow-sm space-y-3">
                  ...
                  <div className="space-y-1">
                    <div className="h-3 w-full bg-emerald-50 rounded-full overflow-hidden border border-emerald-100">
                      <div
                        ...
                        style={{ width: currentStep === 2 ? '78%' : '73%' }}
                      />
                    </div>
                  ...
                  <p className="text-[11px] text-neutral-500 font-medium">
                    🏆 Nhận đơn này để tích lũy thêm{' '}
                    <span className="font-bold text-emerald-700">2.5kg</span> giúp đạt mốc giải cứu
                    50kg trong tháng!
                  </p>
                </div>
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-neutral-700 leading-normal">
                        {reservation.listing.provider.address}
                      </p>
                    </div>
                    <div>
      ...
      {isChatOpen && (
        ...
        >
          <div
            className="bg-white w-full max-w-md h-full flex flex-col shadow-2xl animate-slide-in-from-right"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Chat Header */}
            <div className="p-4 border-b border-neutral-200 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-white/15 text-white flex items-center justify-center text-xs font-extrabold border border-white/20">
                  {initials(reservation.delivery?.shipper?.user?.fullName)}
                </div>
                <div>
              ...
              {chatHistory.map((chat, idx) => {
                ...
                return (
                  <div key={idx} className={`flex ${isShipper ? 'justify-start' : 'justify-end'}`}>
                    <div
                      className={`max-w-[75%] px-3.5 py-2.5 rounded-2xl text-xs leading-normal shadow-sm ${
                        isShipper
        ...
        >
          ...
          >
            <div className="px-5 pt-5 pb-4 bg-brand-gradient relative shrink-0 rounded-t-2xl">
              ...
              <h3 className="font-extrabold text-white text-base pr-8">
                Xác minh danh tính để hoàn tất
              </h3>
            </div>
            <div className="overflow-y-auto flex-1 min-h-0">
              <div className="px-5 py-4 space-y-3">
                ...
                <CameraCapture
                  ...
                  hint={
                    proofMode === 'face'
                      ? 'Chụp ảnh khuôn mặt của bạn để đối chiếu với ảnh đã đăng ký'
                      : 'Chụp ảnh CCCD/CMND của bạn'
                  }
            ...
            <div className="overflow-y-auto flex-1 min-h-0">
              <div className="px-5 py-4 space-y-3">
                <p className="text-sm text-neutral-600 leading-relaxed">
                  Bạn muốn tiếp tục đợi tình nguyện viên giao hàng, tự đến địa chỉ nhận, hay bỏ luôn
                  đơn này?
                </p>
                <button
            ...
            <div className="bg-neutral-50 rounded-2xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold text-neutral-900">
                  {new Date(effectiveStart).toLocaleTimeString('vi-VN', {
                    hour: '2-digit',
                    minute: '2-digit',
                  })}{' '}
                  -{' '}
                  {new Date(pickupEnd).toLocaleTimeString('vi-VN', {
                    hour: '2-digit',
                    minute: '2-digit',
                  })}
                </span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold text-rose-600">
                  Trước{' '}
                  {new Date(deadlineToArrive).toLocaleTimeString('vi-VN', {
                    hour: '2-digit',
                    minute: '2-digit',
                  })}
                </span>
              ...
              <div className="flex items-start gap-2">
                ...
                <span className="text-sm font-bold text-neutral-900 text-right flex-1">
                  {pickupAddress}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 p-4 rounded-2xl bg-amber-50 border border-amber-200">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-amber-600 mt-1 leading-relaxed">
                  ...
                  , đơn đặt sẽ tự động bị hủy để dành suất cho người khác. Bạn cũng sẽ bị trừ điểm
                  uy tín.
                </p>
              </div>
            </div>
