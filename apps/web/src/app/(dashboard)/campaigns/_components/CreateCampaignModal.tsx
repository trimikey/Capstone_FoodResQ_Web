...
export default function CreateCampaignModal({
  ...
  const steps = [
    { label: 'Thông tin', icon: 'campaign' },
    { label: 'Địa điểm', icon: 'place' },
    { label: 'Nhân lực', icon: 'groups' },
  ...
  const toggleSection = (key: 'shifts' | 'menu' | 'schedule' | 'supplies') => {
  ...
  function validateStep(step: 0 | 1 | 2 | 3): boolean {
    const all = collectValidationErrors();
    const keysByStep: Record<0 | 1 | 2 | 3, string[]> = {
      0: ['title', 'description'],
      1: ['kitchenAddress'],
      2: [
        'scheduledDate',
    ...
    const prefixByStep: Record<0 | 1 | 2 | 3, string[]> = {
      0: [],
      1: [],
      2: ['shifts.'],
      3: ['menu.', 'schedule.', 'supplies.', 'shifts.'],
    };
    const allowed = keysByStep[step];
    const prefixes = prefixByStep[step];
    const stepErrors = Object.fromEntries(
      Object.entries(all).filter(([key]) => allowed.includes(key) || prefixes.some((prefix) => key.startsWith(prefix))),
    );
    setErrors((prev) => {
      const next = { ...prev };
      allowed.forEach((key) => delete next[key]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (currentStep < 3) {
      if (validateStep(currentStep)) {
        ...
        toast.error('Vui lòng kiểm tra lại thông tin ở bước này');
    ...
    if (!validateAll()) {
      ...
      toast.error(next[firstKey] ?? 'Vui lòng kiểm tra lại các trường');
  ...
  function createAutoShift() {
    setOpenSections((prev) => ({ ...prev, shifts: true }));
    const start = f.startTime || '08:00';
    const end = f.endTime || '12:00';
    const generated: typeof shifts = [];
    if (f.chefSlotsNeeded > 0) {
      generated.push({ label: 'Ca bếp', role: 'chef', startTime: start, endTime: end, slotsNeeded: f.chefSlotsNeeded });
    }
    if (f.waiterSlotsNeeded > 0) {
      generated.push({ label: 'Ca phục vụ', role: 'waiter', startTime: start, endTime: end, slotsNeeded: f.waiterSlotsNeeded });
    }
    if (f.shipperSlotsNeeded > 0) {
      generated.push({ label: 'Ca giao nhận', role: 'shipper', startTime: start, endTime: end, slotsNeeded: f.shipperSlotsNeeded });
    }
    if (generated.length === 0) {
      generated.push({ label: 'Ca chính', role: undefined, startTime: start, endTime: end, slotsNeeded: 1 });
    }
    setShifts(generated);
  }

  function applyQuickTemplate(kind: 'hundred' | 'lunch' | 'fixed') {
    if (kind === 'hundred') {
      setF((prev) => ({
        ...prev,
        expectedServings: 100,
        chefSlotsNeeded: Math.max(prev.chefSlotsNeeded, 2),
        waiterSlotsNeeded: Math.max(prev.waiterSlotsNeeded, 3),
        shipperSlotsNeeded: Math.max(prev.shipperSlotsNeeded, 2),
      }));
      if (!menu.length) setMenu([{ name: 'Cơm phần dinh dưỡng', type: 'lunch', plannedServings: 100 }]);
      if (!supplies.length) {
        setSupplies([
          { name: 'Gạo sạch', quantity: 25, unit: 'kg' },
          { name: 'Rau củ', quantity: 15, unit: 'kg' },
          { name: 'Hộp đựng phần ăn', quantity: 100, unit: 'hộp' },
        ]);
      }
      setOpenSections((prev) => ({ ...prev, menu: true, supplies: true }));
    }
    if (kind === 'lunch') {
      setF((prev) => ({ ...prev, startTime: '09:00', endTime: '13:00' }));
      if (!schedule.length) {
        setSchedule([
          { time: '09:00', label: 'Chuẩn bị nguyên liệu' },
          { time: '10:30', label: 'Nấu và đóng gói' },
          { time: '11:30', label: 'Phát cơm trưa' },
        ]);
      }
      setOpenSections((prev) => ({ ...prev, schedule: true }));
    }
    if (kind === 'fixed') {
      if (hasProfileAddress) applyProfileAddress();
      if (!shifts.length) createAutoShift();
      setOpenSections((prev) => ({ ...prev, shifts: true }));
    }
  }

  const previewTitle = f.title.trim() || 'Tên chiến dịch của bạn';
  const previewDate = f.scheduledDate
    ? `${new Date(f.scheduledDate).toLocaleDateString('vi-VN')}${f.endDate ? ` - ${new Date(f.endDate).toLocaleDateString('vi-VN')}` : ''}`
    : 'Chưa chọn ngày';
  const previewTime = f.startTime && f.endTime ? `${f.startTime} - ${f.endTime}` : 'Chưa đủ giờ';
  const staffingTotal = f.chefSlotsNeeded + f.waiterSlotsNeeded + f.shipperSlotsNeeded;
  ...
  return (
    <div className="cm-create-overlay" role="dialog" aria-modal="true" aria-labelledby="cm-modal-title">
      ...
      <form onSubmit={submit} className="cm-create-card cm-create-card--wizard">
        <div className="cm-create-banner cm-wizard-banner">
          <div className="cm-create-banner-icon">
            <span className="material-symbols-outlined">{steps[currentStep].icon}</span>
          </div>
          <div className="min-w-0">
            <p className="cm-create-banner-title">{previewTitle}</p>
        ...
        <div className="cm-wizard-steps" aria-label="Các bước tạo chiến dịch">
          {steps.map((step, index) => (
            <button
              key={step.label}
              type="button"
              onClick={() => {
                if (index <= currentStep || validateStep(currentStep)) setCurrentStep(index as 0 | 1 | 2 | 3);
              }}
              className={`cm-wizard-step ${index === currentStep ? 'cm-wizard-step--active' : ''} ${
                index < currentStep ? 'cm-wizard-step--done' : ''
              }`}
            >
              <span className="cm-wizard-step-number">{index + 1}</span>
              <span className="cm-wizard-step-mobile">Bước {index + 1}/4</span>
              <span className="cm-wizard-step-label">{step.label}</span>
            </button>
          ))}
        </div>

        <div className="cm-modal-body cm-wizard-body">
          {currentStep === 0 ? (
            <div className="cm-wizard-pane cm-wizard-pane--split">
              <div className="cm-wizard-main">
                <div className="cm-form-block">
                  <span className="cm-form-block-label">
                    <span className="material-symbols-outlined">info</span>Thông tin chiến dịch
                  </span>
                  <input
                    value={f.title}
                    onChange={(e) => {
                    ...
                    placeholder="VD: Bữa cơm 0 đồng cho lao động Quận 5"
                    className={inputCls('title', 'cm-input cm-input--large')}
                  ...
                  <textarea
                    ...
                    placeholder="Mô tả ngắn: bạn sẽ phục vụ ai, ở đâu, vì sao chiến dịch này quan trọng?"
                    rows={5}
                    maxLength={5000}
              ...
              <aside className="cm-wizard-preview" aria-label="Preview chiến dịch">
                ...
                <div className="cm-preview-content">
                  <span className="cm-preview-badge">Chờ admin duyệt</span>
                  <h3>{previewTitle}</h3>
                  <p>{f.description.trim() || 'Mô tả sẽ hiển thị tại đây khi bạn nhập nội dung.'}</p>
                </div>
              </aside>
            </div>
          ) : null}
          ...
          {currentStep === 1 ? (
            <div className="cm-wizard-pane">
              <div className="cm-form-block">
                ...
                {f.kitchenAddress.trim() || addressMode === 'custom' ? (
                  <div className="cm-modal-map cm-modal-map--wizard">
          ...
          {currentStep === 2 ? (
            <div className="cm-wizard-pane">
              ...
              <div className="cm-form-block">
                <button type="button" className="cm-accordion-head" onClick={() => toggleSection('shifts')}>
                  <span>
                    <span className="material-symbols-outlined">schedule</span>
                    Ca trực nâng cao
                  </span>
                  <span className="material-symbols-outlined">{openSections.shifts ? 'expand_less' : 'expand_more'}</span>
                </button>
                {openSections.shifts ? (
                  <div className="cm-accordion-panel">
                    ...
                    <div className="mb-3 flex flex-wrap gap-2">
                      <button type="button" className="cm-repeat-add" onClick={createAutoShift}>
                        <span className="material-symbols-outlined text-[15px]">auto_awesome</span>
                        Tạo ca tự động từ thời gian chiến dịch
                      </button>
                    </div>
                    <div className="mb-3">
          ...
          {currentStep === 3 ? (
            <div className="cm-wizard-pane cm-wizard-pane--split">
              <div className="cm-wizard-main">
                <div className="cm-form-block">
                  <span className="cm-form-block-label">
                    <span className="material-symbols-outlined">bolt</span>Mẫu nhanh
                  </span>
                  <div className="cm-template-grid">
                    <button type="button" onClick={() => applyQuickTemplate('hundred')} className="cm-template-btn">
                      Bếp 100 suất
                    </button>
                    <button type="button" onClick={() => applyQuickTemplate('lunch')} className="cm-template-btn">
                      Phát cơm trưa
                    </button>
                    <button type="button" onClick={() => applyQuickTemplate('fixed')} className="cm-template-btn">
                      Nấu tại điểm cố định
                    </button>
                  </div>
                </div>

                <div className="cm-form-block">
                  <button type="button" className="cm-accordion-head" onClick={() => toggleSection('supplies')}>
                    <span>
                      <span className="material-symbols-outlined">inventory_2</span>
                      Vật phẩm cần thiết
                    </span>
                    <span className="material-symbols-outlined">{openSections.supplies ? 'expand_less' : 'expand_more'}</span>
                  </button>
                ...
                <div className="cm-form-block">
                  <button type="button" className="cm-accordion-head" onClick={() => toggleSection('menu')}>
                    <span>
                      <span className="material-symbols-outlined">restaurant_menu</span>
                      Thực đơn dự kiến
                    </span>
                    <span className="material-symbols-outlined">{openSections.menu ? 'expand_less' : 'expand_more'}</span>
                  </button>
                  {openSections.menu ? (
                    <div className="cm-accordion-panel">
                      <p className="text-[11px] text-neutral-500 -mt-1 mb-2">
                        Gợi ý món ăn dựa trên vật phẩm dự kiến cần chuẩn bị.
                      </p>
                      <div className="mb-3">
                        <MenuSuggestions supplies={supplies} expectedServings={f.expectedServings} />
                      </div>
                      <div className="cm-repeat">
                        {menu.map((m, i) => (
                          <div key={i} className="cm-repeat-row cm-repeat-row--menu">
                            ...
                            <select
                              value={m.type}
                              onChange={(e) =>
                                setMenu(menu.map((x, j) => (j === i ? { ...x, type: e.target.value } : x)))
                              }
                              className="cm-input"
                              aria-label="Bữa ăn"
                            >
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setMenu(
                                  menu.map((x, j) =>
                                    j === i
                                      ? { ...x, plannedServings: e.target.value === '' ? undefined : Number(e.target.value) }
                                      : x,
                                  ),
                                )
                              }
                              ...
                              className="cm-input"
                            />
                            <button type="button" onClick={() => setMenu(menu.filter((_, j) => j !== i))} className="cm-repeat-remove" aria-label="Xoá món">
                              <span className="material-symbols-outlined text-[18px]">close</span>
                            </button>
                          </div>
                        ))}
                        <button type="button" onClick={() => setMenu([...menu, { name: '', type: 'lunch' }])} className="cm-repeat-add">
                ...
                <div className="cm-form-block">
                  <button type="button" className="cm-accordion-head" onClick={() => toggleSection('schedule')}>
                    <span>
                      <span className="material-symbols-outlined">schedule</span>
                      Lịch trình hoạt động
                    </span>
                    <span className="material-symbols-outlined">{openSections.schedule ? 'expand_less' : 'expand_more'}</span>
                  </button>
              ...
              <aside className="cm-review-card">
                <span className="cm-preview-badge">Tóm tắt trước khi gửi</span>
                <h3>{previewTitle}</h3>
                <dl className="cm-review-list">
                  <div>
                    <dt>Ngày/giờ</dt>
                    <dd>{previewDate} · {previewTime}</dd>
                  </div>
                  <div>
                    <dt>Địa điểm</dt>
                    <dd>{f.kitchenAddress || 'Chưa nhập địa điểm'}</dd>
                  </div>
                  <div>
                    <dt>Số suất</dt>
                    <dd>{f.expectedServings.toLocaleString('vi-VN')} suất</dd>
                  </div>
                  <div>
                    <dt>Nhân lực</dt>
                    <dd>{staffingTotal} người: {f.chefSlotsNeeded} bếp, {f.waiterSlotsNeeded} phục vụ, {f.shipperSlotsNeeded} giao hàng</dd>
                  </div>
                  <div>
                    <dt>Ảnh bìa</dt>
                    <dd>{imageUrl ? 'Đã có ảnh' : 'Chưa có ảnh'}</dd>
                  </div>
                </dl>
              </aside>
            </div>
        ...
        <div className="cm-modal-footer cm-modal-footer--wizard">
          <button
            type="button"
            onClick={() => (currentStep === 0 ? onClose() : setCurrentStep((currentStep - 1) as 0 | 1 | 2 | 3))}
            className="cm-btn-cancel"
          >
            {currentStep === 0 ? 'Huỷ' : 'Quay lại'}
          </button>
          <button type="submit" disabled={pending} className="cm-btn-submit">
            {pending ? (
              <>
              ...
              <>
                <span className="material-symbols-outlined text-[18px]">{currentStep === 3 ? 'send' : 'arrow_forward'}</span>
                {currentStep === 3 ? 'Gửi yêu cầu duyệt' : 'Tiếp tục'}
              </>
            )}
          </button>
        </div>
