...
export default function VolunteerOffersScreen() {
  ...
  const mapListings = useMemo(
    () => (nearbyListings.data?.items ?? []).filter((item) => Number.isFinite(item.lat) && Number.isFinite(item.lng)),
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <FlashList
        ...
        ListEmptyComponent={offers.length === 0 ? renderEmpty : null}
...
function OffersEmptyState({
  ...
  const hasMap = listings.length > 0;
    ...
    <View style={styles.emptyWrap}>
      <View style={styles.watchCard}>
        <View style={styles.watchHeader}>
          <View>
            <Text style={styles.watchTitle}>Khu vực đang theo dõi</Text>
            <Text style={styles.watchSub}>
              {hasMap ? `${listings.length} điểm cung cấp gần bạn` : 'Theo GPS quanh vị trí hiện tại'}
            </Text>
          </View>
          <View style={styles.watchBadge}>
            <MaterialCommunityIcons name="radar" size={16} color={COLORS.primary} />
            <Text style={styles.watchBadgeText}>5 km</Text>
          </View>
        </View>
        ...
        {hasMap ? (
          <View style={styles.mapFrame}>
            <ListingsMapView
              listings={listings}
              center={center ?? { lat: listings[0].lat, lng: listings[0].lng }}
