...
export function CampaignCard({ campaign, onPress, index = 0 }: Props) {
  ...
  return (
    <FadeInUp delay={Math.min(index, 8) * 45} duration={460}>
      <InteractiveScale onPress={onPress} style={styles.card} accessibilityRole="button">
        <View style={styles.accentRail} />
        <View style={styles.cornerTicket}>
          <MaterialCommunityIcons name="heart-pulse" size={15} color={COLORS.onPrimary} />
        </View>
        ...
        <View style={styles.topRow}>
          ...
          <StatusBadge
            label={sm.label}
            tone={campaign.status === 'cancelled' ? 'danger' : campaign.status === 'completed' ? 'success' : 'info'}
          />
        </View>
        ...
        {slots.length > 0 ? (
          <View style={styles.staffPanel}>
            ...
            <View style={styles.slotsRow}>
              {slots.map((slot) => {
                const full = slot.filled >= slot.needed;
                return (
                  <View key={slot.role} style={[styles.slotChip, full && styles.slotChipFull]}>
                    <Text style={[styles.slotText, full && styles.slotTextFull]}>
                      {slot.label} {slot.filled}/{slot.needed}
                    </Text>
                  </View>
                );
              })}
  ...
  return (
    <View style={styles.metaRow}>
      <MaterialCommunityIcons
        name={icon}
        size={16}
        color={icon === 'calendar-clock' ? COLORS.indigo : COLORS.orange}
      />
